...
export function initRenderer(opts: IOpts = {}): RendererAPI {
  ...
  const mermaidDiagrams: Array<{ code: string; id: string }> = [];
  ...
  const renderer: RendererObject = {
    ...
    code({ text, lang = "" }: Tokens.Code): string {
      if (lang.startsWith("mermaid")) {
        ...
        mermaidDiagrams.push({ code: text, id });
        // 返回占位符，后续替换为 PNG
        return `<figure class="mermaid-figure" data-mermaid-id="${id}"><img src="" alt="mermaid diagram" /><figcaption>加载中...</figcaption></figure>`;
      }
  ...
  return {
    ...
    replaceMermaidPlaceholders(html: string, renderedMap: Map<string, string>): string {
      let result = html;
      for (const [id, pngDataUrl] of renderedMap.entries()) {
        const regex = new RegExp(
          `<figure class="mermaid-figure" data-mermaid-id="${id}"><img src=""[^>]*\\/>([\\s\\S]*?)<\\/figure>`,
          'g'
        );
        result = result.replace(regex, `<figure class="mermaid-figure"><img src="${pngDataUrl}" alt="mermaid diagram" /></figure>`);
      }
      return result;
    },
  };
