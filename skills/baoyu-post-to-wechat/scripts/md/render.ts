...
async function main(): Promise<void> {
  ...
  // 渲染 mermaid 图表为 PNG 并替换占位符
  const mermaidDiagrams = renderer.getMermaidDiagrams();
  let contentHtml = baseHtml;
  if (mermaidDiagrams.length > 0) {
    console.log(`Rendering ${mermaidDiagrams.length} mermaid diagram(s)...`);
    const renderedMap = await renderMermaidDiagrams(mermaidDiagrams);
    contentHtml = renderer.replaceMermaidPlaceholders(baseHtml, renderedMap);
  }

  let content = postProcessHtml(contentHtml, readingTimeResult, renderer);
